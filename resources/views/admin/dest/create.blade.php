    ...
    <div class="flex justify-center">
        <div class="w-full pl-0 mt-6 lg:w-1/2 lg:pl-2">
            ...
            <div class="leading-loose">
                <form class="p-10 bg-white rounded shadow-xl" method="post" action="{{ route('admin.destination.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="">
                        ...
                        <input class="w-full px-5 py-1 text-gray-700 capitalize bg-gray-200 rounded" id="name" name="name" type="text" required="" placeholder="Nama Destinasi" aria-label="Name" value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="mt-2">
                        ...
                        <select name="type" id="type" class="w-full px-5 py-3 text-gray-700 bg-gray-200 border-2 rounded" required>
                            @foreach ($types as $data)
                                <option value="{{ $data->id }}" {{ old('type')==$data->id?"selected":"" }}>{{ $data->name }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <span class="text-sm italic text-red-600">{{ $message }}</span>
                    ...
                    <div class="mt-2">
                        ...
                        <select name="region" id="region" class="w-full px-5 py-3 text-gray-700 bg-gray-200 border-2 rounded" required>
                            <option value="Sibolga" {{ old('region')=="Sibolga"?"selected":"" }}>Sibolga</option>
                            <option value="Tapanuli Tengah" {{ old('region')=="Tapanuli Tengah"?"selected":"" }}>Tapanuli Tengah</option>
                        </select>
                        @error('region')
                            <span class="text-sm italic text-red-600">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="mt-2">
                        ...
                        <input class="w-full px-2 py-2 text-gray-700 capitalize bg-gray-200 rounded" id="address" name="address" type="text" required="" placeholder="Alamat" value="{{ old('address') }}">
                        @error('address')
                            <span class="text-sm italic text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mt-2">
                        ...
                        <textarea class="w-full px-2 py-2 text-gray-700 bg-gray-200 rounded" id="description" name="description" required="" placeholder="Deskripsi" >{{ old('description') }}</textarea>
                        @error('description')
                            <span class="text-sm italic text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mt-2">
                        ...
                        <input class="w-full px-2 py-2 text-gray-700 bg-gray-200 rounded" id="maps_url" name="maps_url" type="text" placeholder="URL" value="{{ old('maps_url') }}">
                        @error('maps_url')
                            <span class="text-sm italic text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
